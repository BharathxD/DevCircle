...
const forumUpdateValidator = object({
  forumId: string({ required_error: "Forum id is required" }),
  forumName: string({ required_error: "Name is required" }).min(3, { message: "Forum name must be atleast 3 characters long" }).max(21, {
...
export { forumValidator, forumSubscriptionValidator, forumUpdateValidator };
export type { CreateForumPayload, ForumSubscriptionPayload, ForumUpdatePayload };
