          ...
          >
            ...
            {posts?.map((post) => {
              return (
                <a
                  key={post.id}
                  className="inline-flex items-center justify-between gap-2 overflow-hidden rounded-md border-2 border-zinc-800 bg-zinc-50 hover:bg-zinc-300 dark:bg-zinc-950 dark:hover:bg-zinc-800"
                  href={`/d/${post.forum.name}/post/${post.id}`}
                >
                  <h2 className="px-4 py-2">{post.title}</h2>
                  <time className="hidden h-full items-center justify-center bg-zinc-300 px-4 py-2 dark:bg-zinc-800 md:flex">
