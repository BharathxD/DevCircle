...
const getUsers = async (req: NextRequest): Promise<NextResponse> => {
  try {
    ...
    const posts = await database.$queryRaw(
      Prisma.sql`SELECT P.id as postId, P.title as postTitle, P.createdAt, F.id as forumId, F.name as forumName FROM Post P JOIN Forum F ON P.forumId = F.id WHERE P.title LIKE ${`%${queryParam}%`}`
    );
