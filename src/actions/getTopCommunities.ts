 ...
 */
const fetchTopCommunities = async (): Promise<{ forumName: string; memberCount: number }[] | null> => {
  try {
    ...
    return Object.values(cachedTopCommunities) as { forumName: string; memberCount: number }[];
  } catch (error) {
