 ...
 */
const getSitemapData = async (): Promise<{ posts: ReturnType[], forums: ReturnType[], users: ReturnType[] }> => {
    const postsData = await database.post.findMany({ include: { forum: true } });
    const forumData = await database.forum.findMany();
    const userData = await database.$queryRaw`SELECT username FROM User` as { username: string }[];
